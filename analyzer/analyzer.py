...
class Analyzer:
    ...
    def format_result_for_visualizer(self, data):
        ...
        for data_group in ["summary", "detail"]:
            ...
            for key, value in output_sort["ceph"]["cpu"][data_group].items():
                for name, data in value.items():
                    cpu_core_data_dict[name] = data
            output_sort["ceph"]["cpu"][data_group] = cpu_core_data_dict
            cpu_core_data_dict = OrderedDict()
            for key, value in output_sort["client"]["cpu"][data_group].items():
                for name, data in value.items():
                    cpu_core_data_dict[name] = data
            output_sort["client"]["cpu"][data_group] = cpu_core_data_dict
        return output_sort
