...
class database(object):
    ...
    def insert_to_TB(self,data,dbpath):
        ...
        if len(rowdata) == 19:
            ...
            sqlstr = "insert into tb_report (runid,runid_tr,timestamp,status,description,opsize,optype,qd,driver,snnumber,cnnumber,worker,runtime,iops,bw,latency,latency_99,sniops,snbw,snlatency) values ("+rowdata[1]+",'"+rowdata[0]+"','"+rowdata[2]+"','"+rowdata[3]+"','"+rowdata[4]+"','"+rowdata[5]+"','"+rowdata[6]+"','"+rowdata[7]+"','"+rowdata[8]+"',"+rowdata[9]+","+rowdata[10]+","+rowdata[11]+","+rowdata[12]+",'"+rowdata[13]+"','"+rowdata[14]+"','"+rowdata[15]+"','"+rowdata[18]+"','"+rowdata[19]+"','"+rowdata[20]+"','"+rowdata[21]+"')"
    ...
    @classmethod
    def select_report_list(self, dbpath, selected_lines):
        if len(selected_lines) == 0:
            sqlstr = "select * from tb_report"
        else:
            cmd = []
            for runid in selected_lines:
                print runid
                cmd.append("runid = '%s'" % runid)
            sqlstr = "select * from tb_report where %s" % " or ".join(cmd)
